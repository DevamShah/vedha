// Vedha — Autonomous AI Pentester (fork of Shannon by Keygraph, AGPL-3.0).
// Upstream copyright is preserved on inherited files; this file is new in
// Vedha and added under the same AGPL-3.0 license.

/**
 * SarifReportOutputProvider — emits a SARIF 2.1.0 file alongside the
 * assembled markdown report.
 *
 * Activated by setting `VEDHA_REPORT_FORMAT=sarif` on the worker. The
 * provider runs via the standard `ReportOutputProvider` seam (inherited
 * from upstream Shannon) after the markdown report has been written.
 *
 * Scope (v0.1):
    ...
    runs: [
      {
        tool: {
          driver: {
            name: 'Vedha',
            informationUri: 'https://github.com/DevamShah/vedha',
...
export class SarifReportOutputProvider {
  constructor(private readonly driverVersion?: string) {}

  async generate(input: SarifProviderInput, logger: ActivityLogger): Promise<{ outputPath?: string }> {
